...
1. packages (Internet Plans)
   ...
   - package_name: VARCHAR(50) NOT NULL UNIQUE (e.g., "Basic 10M", "Ultra 20M")
...
2. customers (Client Directory)
   ...
   - created_at: TIMESTAMPTZ DEFAULT NOW()
   - Add indexes on phone_number, full_name, and status for rapid searching across 50,000+ records.
...
3. payments (Financial Ledger)
   ...
   - payment_month: VARCHAR(20) NOT NULL (e.g., "August 2026")
   - payment_method: VARCHAR(30) NOT NULL CHECK (payment_method IN ('Cash', 'EasyPaisa', 'JazzCash', 'Bank'))
...
5. Row Level Security (RLS) & Triggers:
   ...
   - Add automatic updated_at triggers for tables with modification tracking.
...
│   └── layout.tsx
├── components/
│   ├── ui/                             # shadcn components (Button, Input, Table, Badge, Card, Select, Dialog)
...
- Global Search Bar: Instantly filter active database records by Name, Phone Number, CNIC, or Address with live debounced search (< 200ms latency).
...
- CSV Parser validating required headers (full_name, phone_number, cnic_number, address, package_name, status).
...
- Python CSV Migration script (scripts/migrate_paper_logs.py) using supabase-py for automated bulk data ingestion as specified in Section 6.
